   ...
   if (pageid == 'exercise') {
		...
		$(function () {
			$('#timer .startButton').click(function () {
				timerInstance.start({countdown: true, startValues: {seconds: 300}});
				$(this).addClass('off');
				$('#timer .pauseButton').removeClass('off');		
				$('#timer .timer__count').removeClass('off');
			});

			$('#timer .pauseButton').click(function () {
				timerInstance.pause();
				$(this).addClass('off');
				$('#timer .startButton').removeClass('off');		
			});
			timerInstance.addEventListener('secondsUpdated', function (e) {
				$('#timer .values').html(timerInstance.getTimeValues().toString());
			});
			timerInstance.addEventListener('targetAchieved', function (e) {
				$('#timer .values').html('Done!');
			});
		});	
   }			
/*-------------- Page Audio----------- */
